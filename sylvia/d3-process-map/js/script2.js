...
function drawGraph() {
    ...
    graph.legend.append('circle')
        .attr('cx', graph.legendConfig.xOffset)
        .attr('cy', function(d, i) {
            return graph.legendConfig.yOffset + i * graph.legendConfig.lineHeight;
        })
        .attr('r', graph.legendConfig.circleR)
    ...
    graph.nodeCirc = graph.node.append('circle')
        /*.attr('rx', 5)
        .attr('ry', 5)
        .attr('stroke', function(d) {
        ...
        .attr('r' , 30); /* WHAT DOES THIS DO */

    graph.node.each(function(d) {
        var node  = d3.select(this),
            circ  = node.select('circle'),
            lines = wrap(d.name),
            ddy   = 1.1,
...
function selectObject(obj, el) {
    ...
    if (nodeCirc.left < graphRect.left ||
        nodeCirc.top  < graphRect.top  ||
        nodeCirc.left + nodeCirc.width  > graphRect.left + graphRect.width ||
        nodeCirc.top  + nodeCirc.height > graphRect.top  + graphRect.height) {

        $graph.animate({
            scrollLeft : nodeCirc.left + nodeCirc.width  / 2 - graphRect.width  / 2,
            scrollTop  : nodeCirc.top  + nodeCirc.height / 2 - graphRect.height / 2
        }, 500);
    }
}
