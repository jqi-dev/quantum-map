...
function drawGraph() {
    ...
    graph.svg.append('defs').selectAll('marker') /* arrow */
        .data(['end'])
        .enter().append('marker')
        .attr('id', String)
    ...
    graph.legend.append('circle')
        .attr('cx', graph.legendConfig.xOffset / 2)
        .attr('cy', function(d, i) {
            return graph.legendConfig.yOffset + 6 + i * graph.legendConfig.lineHeight;
        })
        .attr('r', graph.legendConfig.rectHeight / 2)
    ...
    setTimeout(function() {
        graph.node.each(function(d) {
            ...
            node.select('circle')
                .attr('cx', bounds.x1 + 20)
                .attr('cy', bounds.y1 + 18.5)
                .attr('r', 22.5);
...
function showDescription(obj, el) {
...
$('#search-button').on("click", function() {
    ...
    if (searchedEl) {
        if (!highlighted || searchedObj != highlighted) {
            selectObject(searchedObj, searchedEl);
        }
    } else {
